...
class ExportTaskView(gui3d.TaskView):
    def __init__(self, category):
        ...
        def onFileSelected(event):
            ...
            characterPath = '/home/exp/Documents/makehuman/v1py3/models/'
            for i in range( 0,3, 1):
                for k in range(0,1):
                    j=0
                    try:
                        gui3d.app.loadHumanMHM(characterPath+setarr[j]+str(i+k)+'.mhm')
                    except: continue
                    onFileSelectedCopy(characterPath+'3D/'+setarr[j]+str(i+k))

            end_time=time.time()
            print('running time: ', end_time-start_time)
            # onFileSelectedCopy(event.path)
        def onFileSelectedCopy(path):
